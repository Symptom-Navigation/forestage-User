...
import serviceGuide from "../utils/request3";
...
const symptomHandler = async () => {
  if (symptom.value) {
    messages.value.push({ id: 1, type: "text", text: symptom.value });
    messageInput.value = "";

    if (messageCount === 0) {
      try {
        const res = await serviceGuide.post("/symptom", {
          username: "1234567",
          sessionID: 1,
        ...
        if (res.data.options && res.data.options.length > 0) {
          combinedMessage +=
            res.data.options.join("\n") +
            "\n\n*************************************************\n\n";
        }
        if (res.data.diseases && res.data.diseases.length > 0) {
          combinedMessage +=
            res.data.diseases.join("\n") +
            "\n\n*************************************************\n\n";
        }
        if (res.data.departments && res.data.departments.length > 0) {
          combinedMessage +=
            res.data.departments.join("\n") +
            "\n\n*************************************************\n\n";
        }
        if (!res.data.diseases && !res.data.departments && !res.data.options) {
          combinedMessage = "您好，我没理解您的意思";
        }
        ...
        if (combinedMessage) {
          responseMessages.value.push({
            id: -1,
            type: "text",
            text: combinedMessage.trim(), // 去掉最后一个多余的换行符
          });
        }

        if (responseMessages.value.length > 0) {
          displayMessage(responseMessages.value[0].text);
        }
      } catch (error) {
      ...
      const nextMessage = responseMessages.value[messageCount];
      if (nextMessage) {
...
const parseMarkdown = (markdown: string) => {
  ...
  sections.forEach((section) => {
    ...
    parsedData[title] = lines.join("<br>");
  });
...
const symptomHandler = async () => {
  if (symptom.value) {
    ...
    eventSource.addEventListener("message", (event) => {
      const data = event.data;
      const parsedData = parseMarkdown(data.trim());
      Object.keys(parsedData).forEach((key) => {
        displayMessage(`### ${key}<br>${parsedData[key]}`);
      });
    });
